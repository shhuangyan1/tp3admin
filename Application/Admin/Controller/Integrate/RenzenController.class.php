...
class RenzenController extends  BaseController
{
    ...
    public function DataList(){
        ...
        $where['Type']=4;  //平台类型  0 短信 1 支付 2 第三方登录 3存储接口 4认证接口
        ...
        if($array['rows']){
            foreach ($array['rows'] as $val){
                $val['Status']=$query->GetValue(self::T_DICTIONARY,array('DictType'=>'2','DictValue'=>$val['Status']),'DictName');
    ...
    public function Edit($ID=null){
        ...
        $this->display('Integrate/Renzen/Edit');
    }
    ...
    public function Save(){
        if(IS_POST){
            //数据保存前的验证规则
            $rules = array(
                array('Name','require','接口名称必须填写！'), //默认情况下用正则进行验证
                array('EName','require','接口英文名称必须填写！'), //默认情况下用正则进行验证
            );
            ...
            if(!$data){
                ...
                //记录操作者信息
                $data['Type']=4; // 平台类型 0 短信 1 支付 2 第三方登录
                ...
                if($data['ID']>0){
                    ...
                    $where['Type']=4;  // 平台类型 0 短信 1 支付 2 第三方登录
                    $con1['Name']=$data['Name'];
                    $con2['EName']=$data['EName'];

                    $NameExists=M(self::T_TABLE)->where($where)->where($con1)->count();
                    if($NameExists>0){
                        $this->ajaxReturn(false, '接口名称已存在！');
                    }
                    $ENameExists=M(self::T_TABLE)->where($where)->where($con2)->count();
                    if($ENameExists>0){
                        $this->ajaxReturn(false, '接口英文名称已存在！');
                    }

                    // if($data['IsDefault']==1){
                    //     $val['IsDefault']=0;
                    //     $model->where(array('Type'=>4,'IsDefault'=>1))->save($val);
                    // }
                    ...
                    $NameExists=M(self::T_TABLE)->where(array('Name'=>$data['Name'],'Type'=>4))->count();
                    if($NameExists>0){
                        $this->ajaxReturn(false, '接口名称已存在！');
                    }
                    $ENameExists=M(self::T_TABLE)->where(array('EName'=>$data['EName'],'Type'=>4))->count();
                    if($ENameExists>0){
                        $this->ajaxReturn(false, '接口英文名称已存在！');
                    }
                    // if($data['IsDefault']==1){
    ...
    public function Del()
    {
        ...
        foreach ($arr as $val){
            ...
            if($IntegrateIDExists>0){
                $this->ajaxReturn(false, "该平台接口下存在参数信息，禁止删除！");
            }
        }
        //判断完成，开始执行物理删除
