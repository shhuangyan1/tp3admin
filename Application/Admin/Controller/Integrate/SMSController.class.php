...
class SMSController extends  BaseController
{
    ...
    public function DataList(){
        ...
        $where['Type']=0;  //平台类型  0 短信 1 支付 2 第三方登录
    ...
    public function Edit($ID=null){
        ...
        $this->display('Integrate/SMS/Edit');
    }
    ...
    public function Save(){
        if(IS_POST){
            ...
            if(!$data){
                ...
                //记录操作者信息
                $data['Type']=0; // 平台类型 0 短信 1 支付 2 第三方登录
                ...
                if($data['ID']>0){
                    ...
                    if($data['IsDefault']==1){
                        $val['IsDefault']=0;
                        $model->where(array('Type'=>0,'IsDefault'=>1))->save($val);
                    }
                    ...
                    $NameExists=M(self::T_TABLE)->where(array('Name'=>$data['Name'],'Type'=>0))->count();
                    if($NameExists>0){
                        $this->ajaxReturn(false, '平台名称已存在！');
                    }
                    $ENameExists=M(self::T_TABLE)->where(array('EName'=>$data['EName'],'Type'=>0))->count();
                    if($ENameExists>0){
                        $this->ajaxReturn(false, '平台英文名称已存在！');
                    }
                    if($data['IsDefault']==1){
