 ...
 */
 ...
 use XBCommon;
 class RenewsetController extends BaseController{

     const T_TABLE='loans_renewset';
     ...
     public function Save(){
         ...
         if(!$FormData){
             ...
             $where['Nums']=$FormData['Nums'];
             $where['IsDel']=0;
             $NameExist=$model->where($where)->find();
             if($NameExist){
                 $this->ajaxReturn(0,'次数已存在');
             }
             //保存数据
             $data=$FormData;
