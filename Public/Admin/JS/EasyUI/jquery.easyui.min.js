...
$.extend($.fn.validatebox.defaults.rules, {
    oneparam: {//动态Ajax验证，用法:<input id="CardID" name="CardID" type="text" class="easyui-textbox" data-options="required:true,validType:['cardid',&#34 oneparam['/Member/management/CardID','@ViewData["ID"].ToString()','身份证号码已被注册！'] &#34]" />
        validator: function (value, param) {
            var result = true;
            $.ajax({
                url: param[0],
                data: { 'Value': value, 'param': param[1] },
                type: "POST",
                async: false,
                cache: false,
                success: function (data) {
    ...
    telormobile: {
        validator: function (value, param) {
            var reg = /(^0\d{2,3}-?\d{7,8})|(^0?1[3|4|5|7|8][0-9]\d{8})$/;
            return reg.test(value);
        },
        message: '电话号码不正确'
    },
    ...
    cardid: {
        validator: function (value, param) {
            ...
            idCard = value.replace(/ /g, "").replace(/(^\s*)|(\s*$)/g, "");               //去掉字符串头尾空格                     
            if (idCard.length == 18) {
